...
class account {
  Future<String> update_user_account(
...
// message=responsejeson['success'];
  ...
  Future<String> resetPassword(String pass, String c_pass) async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${baseUrl}/api/Admin/ResetPassword'));
    request.fields.addAll({'password': pass, 'c_password': c_pass});
  ...
  static Future<String> ResetPasswordRequest() async {
    var response = await http
        .get(Uri.parse('${baseUrl}/api/ResetPasswordRequest'), headers: {
      'Accept': 'application/json',
