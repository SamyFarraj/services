...
class _BlockServiceState extends State<BlockService> {
  ...
  _builderFunction(context, snapshot, state, cubit) {
    if (snapshot.hasData) {
      ...
      woodWardServices.add('Select Service');
      bothStreetsServices.add('Select Service');
  ...
  Widget _streetDropDownMenu() => SizedBox(
        ...
        child: DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: selectedService != 'Select Service' ? OutlineInputBorder(
          ...
          items: selectStreet
              .map(
                (street) => DropdownMenuItem<String>(
                  enabled: selectedService == 'Select Service',
                  value: street.title,
                  child: _text(
                    street.title,
                  ),
  ...
  _onChangedServices(service, snapshot) => setState(() {
        ...
        if (selectedStreet == 'WOODWARD') {
          ...
          for (int i = 0; i < snapshot.data!.services.bothStreet.length; i++) {
            if (selectedService == snapshot.data!.services.bothStreet[i].name) {
              serviceId = snapshot.data!.services.bothStreet[i].id;
